...
\begin{abstract}
  ...
  \vb{The majority of empirical population genetic
  studies have tried to understand the evolutionary processes that have shaped
...
\vb{Our goal is to understand how linked selection affects the frequency
...
\end{align}
%
\vb{where, $\Delta_{_N} p_t$, $\Delta_{_M} p_t$, and $\Delta_{_H} p_t$ are the
...
\emph{heritable change} in neutral allele frequency), respectively. Note
...
\end{align}
%
\vb{The terms $\var(\Delta_{_N} p_t)$ and $\var(\Delta_{_M} p_t)$ capture the
...
\end{align}
%
\vb{These covariance terms are expected to be non-zero only when there is heritable
...
an advantageous site, quantitative genetic models of linked selection considered
...
basis and simplifying (see Appendix Section \ref{ap:multilocus} for details)
gives,
...
genetic variance and the drift-effective population size. \vb{We focus on this
...
sample allele frequencies for a sample of size $n=100$ chromosomes.}
...
\vb{As the number of empirical temporal genomic studies continue to increase,
  ...
  impacted by the heritable variance in offspring number, $\var(\Delta_{_H} p_t)$
  ...
  10 through 14, and so forth could serve as a compromise. Second, as described
  ...
  introduces additional uncertainty into our parameter estimates. In cases
  where the experimenter suspects \emph{a priori} that fluctuating selection is
