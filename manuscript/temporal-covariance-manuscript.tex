...
allele frequency change across generations. We can see how this covariance
contributes to the total cumulative variance in allele frequency change over
$t$ generations by writing the total allele frequency change as the sum of
allele frequency changes each generation,
...
\begin{align}
  ...
  \mathcal{C}(V_a, R, t, s) := \frac{\E_n(\cov(\Delta p_t, \Delta p_s))}{\E_n(p_{t} (1-p_{t}))} &= \frac{V_a(s)}{2} \underbrace{\int_0^R \E(\mathcal{R}_t^2(r(c))) \; e^{-r(c)(s-t)} \frac{2(R-c)}{R^2} \,d c}_{\mathcal{A}(R, t, s)} 
\end{align}
%
...
$V_a(s)$ match those of $V_A(s)$ when $s = 6$ (see also appendix Figure
...
variance across $t$ generations is $\var(p_t - p_0) \approx (1 -
\exp(-\nicefrac{t}{2N})) \approx p_0(1-p_0) \times \nicefrac{t}{2N}$, where this second
approximation works for short time spans ($\nicefrac{t}{2N} \ll 1$).  This
suggests that we can use our method-of-moments estimate of the effective
population size without the effects of selection, $\widehat{N}$, and compare
the fraction of standardized variance we expect under this rate of drift to the
empirical standardized variance,
...
All code and simulation data to reproduce these results is available on GitHub
...
Chuck Langley, Sally Otto, Jonathan Pritchard, Anita To, Michael Turelli, and
members of the Coop lab for helpful conversations. This research was supported
by an NSF Graduate Research Fellowship grant awarded to VB (1650042), and an
NIH (R01-GM108779) and NSF (1353380) grants awarded to GC.
...
\begin{align}
  ...
  &= \frac{V_a(s)}{2} \int_0^R \E(\mathcal{R}_t^2(r(c))) \; (1-r(g))^{(s-t)} \frac{2(R-c)}{R^2} \,d c \\
...
$t$). However, we do not \emph{a priori} know whether a site is selected or
not, so we sum over all $M$ loci, thus characterizing the average linkage
disequilibria in a region as
...
dynamics at the selected loci are not explicitly modeled. In contrast, in
deriving our expressions for temporal autocovariance and variances, we use a
population genetic approach, modeling the dynamics at selected sites (though we
simplify from the full multilocus treatment, e.g. we assume selected loci
experience independent sweeps and we ignore the LD between selected sites). We
show that we can reconcile the two approaches, and demonstrate that the
temporal autocovariance expressions we develop in our models are implicit in
their model.
