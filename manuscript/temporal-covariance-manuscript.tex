...
frequency change: a neutral allele may become randomly associated with the
genetic backgrounds that determine the fitness differences between individuals,
and consequently the neutral allele's frequency changes is impacted by the
fitness of these backgrounds. This is a form of linked selection which has
traditionally been modeled as a reduction in the long-run effective population
size where an equilibrium between XXX

as fitness-determining alleles persist on their backgrounds until
disassociated by segregation and recombination \parencite{Robertson1961-ho,
Santiago1995-hx,Santiago1998-bs}. Like drift, this form of linked selection has
effect on the net direction of neutral allele frequency change ($\E(\Delta p_t)
= 0$) because the set of high- or low-fitness genetic backgrounds a neutral
allele resides on is random. However, 

the higher variance
leads the neutral allele to experience a faster rate of \emph{apparent}
`drift'. This has traditionally been thought of as a reduction in the long-run
effective population size, as fitness-determining alleles persist on their
backgrounds until disassociated by segregation and recombination
\parencite{Robertson1961-ho, Santiago1995-hx,Santiago1998-bs}. 


neutral alleles recombine onto, or segregate with the genetic
backgrounds that determine the fitness differences between individuals, and
these random associations with fitness backgrounds affect neutral allele
frequency change as high fitness backgrounds leave more descendants, and low
fitness backgrounds leave fewer. 


While classic hitchhiking
theory \parencite{Maynard_Smith1974-lc} models how neutral alleles are dragged
towards fixation by new beneficial alleles, we extend a body of theory that
models linked selection from a quantitative genetics framework
\parencite{Robertson1961-ho,Santiago1995-hx,Santiago1998-bs}. In this
framework, a large number of polymorphisms contribute to the heritable fitness
differences between individuals i.e. the additive genetic variance for
offspring number. 
...
We can partition
a neutral allele frequency's change into three uncorrelated stochastic
components (following \citeauthor{Santiago1995-hx}, \citeyear{Santiago1995-hx},
see our Appendix Section \ref{ap:decomp} for proof),
...
\begin{table}[]
 ...
 $\Delta_{_N} p_t $ & Frequency change due to non-heritable variation in fitness  \\
 $\Delta_{_M} p_t $ & Frequency change due to Mendelian segregation \\
 $\Delta_{_H} p_t $ & Frequency change due to heritable differences \\
 $N$ & Census population size of breeding individuals \\
 $N_e$ & Effect population size \\
 $f_i$ & Fitness (expected number of offspring) of individual $i$\\
 $\alpha_{t,l}$ & Effect size in generation $t$ and locus $l$ \\
 $L$ & Total number of loci impacting fitness \\
 $g_{i,l} \in \{0, 1, 2\}$ & Individual $i$'s gene count at locus $l$ \\
 $x_{i} \in \{0, 1, 2\}$ & Individual $i$'s neutral gene count at the tracked neutral site\\
 $D_{t,l}$ or $D_{t,l}'$ & Gametic linkage disequilibrium between the tracked neutral site and selected locus $l$ at time $t$ (see Supplementary Figure \ref{fig:ld-cartoon}) \\
 $D_{t,l}''$ & Non-gametic disequilibrium between the tracked neutral site and selected locus $l$ at time $t$ (see Supplementary Figure \ref{fig:ld-cartoon}) \\
 $\E(\mathcal{R}_{t,l}^2)$ & The squared correlation coefficient of linkage disequilibrium between the tracked neutral site and selected site $l$ at time $t$\\
 $r_l$ & The recombination fraction between the tracked neutral site and selected site $l$\\
 $V_a(s)$ & The additive genic variance \\ 
 $V_A(s)$ & The additive genetic variance \\ 
 $R$ & The total level of recombination in the region, in Morgans \\ 
 $r(g)$ & A mapping function (i.e. Haldane's), which maps a position $g$ to a recombination fraction.\\ 
 $\rho$ & The population recombination rate, $\rho = 4Nr$\\ 
 $\mathcal{A}(R, t, s)$ & The average linkage disequilibrium in a region of $R$ Morgans, that persisted from generation $t$ to generation $s$. \\
 ...
 $ssh_n(t)$ & The proportion of sum of site heterozygosity at neutral sites at time $t$ relative to $SSH(1)$, $SSH_n(t) / SSH_n(1)$ \eqref{eq:VA-ssh} \\
 $z_i$ & The breeding value of the trait that determines fitness, $z_i = \sum_{i=1}^L \alpha g_{i,l}$, see Appendix Section \ref{ap:multilocus} \\
 $w(z_i)$ & The fitness of individual $i$ with fitness function $w(\cdot)$ \\
 $\mathbf{\widehat{Q}}$ & The sample standardized variance-covariance matrix \\
 $q_{t,s}$ & The elements of the matrix $\mathbf{\widehat{Q}}$  \\
 $\Delta p_{n,t}$ & The allele frequency change at site $n$ between times time $t+1$ and $t$, \eqref{eq:ave-temp-autocov} \\
 $\tau$ & The number of allele frequency changes observed, e.g. after sampling for $\tau + 1$ timepoint\\
 $V_{a,ssh_n}(t)$ & The additive genic variation at time $s$ as approximated by the observed decay in the sum of site heterozygosity at neutral sites  \\
