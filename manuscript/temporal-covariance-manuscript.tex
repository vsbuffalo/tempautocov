...
\begin{abstract}
  ...
  backgrounds \vb{reduces} \vince{didn't really know what Nick was looking for
  here?} effective population experienced by this neutral allele
...
\emph{heritable change} in neutral allele frequency), respectively. Note
  ...
  adjacent generations, that \vince{see Nick's comment - is this where the "that" goes?} the same approach can be extended to situations where
...
that show temporal autocovariance is created under both tight and loose linked
selection, and below, develop expressions for its magnitude that are applicable
to both, bridging \vince{what's nick's comment?}the two regimes of linked
selection.
...
\vince{I can't get at Nick's comment here. I've looked at the paper, but don't
see a connection?}
...
occurs in models of recurrent sweeps \parencite{Stephan1992-jc} and models
...
\texttt{msprime} was called are available on \vb{our} Github repository
...
the dynamics of \vb{the additive genetic variance associated with a particular
fitness background through time} by using the empirically observed changes in
sum of site heterozygosity in a region as a proxy for its decay. While we have
shown this model works under directional selection in a relatively idealized
setting, inference in natural populations may be complicated by changes in the
...
\vb{Additionally we can consider the impact that other linked selected sites
  have on the neutral allele's frequency trajectory. From
  equation \eqref{eq:integral-overselpos}, we can take a piecewise integral
  over all linked sites (those less than a recombination fraction of one-half
  away), and all unlinked sites (those at a recombination fraction of
one-half away).}

  \begin{align}
    \frac{\cov(\Delta p_t, \Delta p_s)}{p_t(1-p_t) } &= \frac{V_a(s)}{2R} \int_{-\nicefrac{R}{2}}^{\nicefrac{R}{2}} \E(\mathcal{R}^2_{t}(r(g))) (1-r(g))^{s-t} d g \\
                                                     &= \frac{V_a(s)}{2R} \left[ 2 \int_{0}^{\nicefrac{1}{2}} \E(\mathcal{R}^2_{t}(r)) (1-r)^{s-t} d r  + 2 \frac{\E(\mathcal{R}^2_{t}(\nicefrac{1}{2}))}{2^{s-t}} (R - g(\nicefrac{1}{2})) \right] 
  \end{align}
\vb{
Here, $g(r)$ is the inverse function of $r(g)$ and $R$ is the total map length
of the chromosome that contained the focal neutral site. The second term in the
braces are the two unlinked selections of this chromosome, e.g.
$[-\nicefrac{R}{2}, -g(\nicefrac{1}{2})]$ and $[g(\nicefrac{1}{2}),
\nicefrac{R}{2}]$ (this is zero if $g(\nicefrac{1}{2}) > \nicefrac{R}{2}$).
With this construction, we can then consider the contribution of additive
variation on the $M$ Morgans on other chromosomes of a genome of $M + R$
Morgans of total length.
}

\begin{align}
    \frac{\cov(\Delta p_t, \Delta p_s)}{p_t(1-p_t) } &= \frac{V_a(s)}{2R} \left[ 2 \int_{0}^{\nicefrac{1}{2}} \E(\mathcal{R}^2_{t}(r)) (1-r)^{s-t} d r  + \frac{\E(\mathcal{R}^2_{t}(\nicefrac{1}{2}))}{2^{s-t}} (M + 2R - 2g(\nicefrac{1}{2})) \right] 
\end{align}

\vb{Ideally, we could evaluate the inverse mapping function at $\nicefrac{1}{2}$ to
find what genetic distance a site becomes unlinked to the focal neutral site.
However, mapping functions asymptotes to $\nicefrac{1}{2}$, so here
calculations, one may want to assume $0.49$ is approximately unlinked. 

As a simple thought experiment, we can imagine the focal chromosome is exactly
$2 g(\nicefrac{1}{2})$ Morgans long, so that we just have to consider the
impact of variation on other chromosomes. Then we might ask for what value of
$M$ does the contribution from unlinked sites dominate the contribution from
linked sites?}
